...
All of the following data is stored **only on your device** using Apple's SwiftData. None of it is transmitted to Dayfare's servers, because Dayfare has no servers.
...
The app makes a single outbound network request to `open.er-api.com` to fetch public currency exchange rates. No personal or device data is sent with this request.

## Data sharing

Dayfare does not share your data with anyone, because Dayfare never receives it.
